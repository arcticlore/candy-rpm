...
- ~~diagon~~ — cmake тянет git-сабмодули — COPR без сети; ждать vendored release
- ~~cli-visualizer~~ — Апстрим недоступен (404) / пакет удалён из registry
- ~~linuxwave~~ — Апстрим полностью на Zig (сборка build.zig) — компилятора нет в Fedora. Вернуть, когда появится zig или офиц. RPM
