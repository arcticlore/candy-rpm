...
FAILED=$(copr-cli list-builds arcticlore/candy 2>/dev/null \
...
BODY=$(jq -n --arg t "$TITLE" --arg list "$(echo "$FAILED" | sed 's/^/- /' | tr '\n' '\n')" \
            --arg url "$(copr-cli list-builds arcticlore/candy >/dev/null 2>&1; echo https://github.com/arcticlore/candy-rpm/actions)" \
