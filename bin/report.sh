...
copr-cli list-builds arcticlore/terminal-rpm 2>/dev/null | awk '{print $NF}' | sort | uniq -c | sort -rn | awk '{printf "- %s: %s\n",$2,$1}'
echo
echo "## Зелёные пакеты (последний билд)"
copr-cli list-builds arcticlore/terminal-rpm 2>/dev/null | awk '!seen[$2]++ && $NF=="succeeded"{print $2}' | sort | awk '{printf "- %s\n",$0}'
echo
echo "## Красные пакеты (последний билд)"
copr-cli list-builds arcticlore/terminal-rpm 2>/dev/null | awk '!seen[$2]++ && $NF=="failed"{print $2}' | sort | awk '{printf "- %s\n",$0}'
echo
...
if [ -n "$TOKEN" ]; then
  curl -s --max-time 20 -H "Authorization: Bearer $TOKEN" \
    "https://api.github.com/repos/arcticlore/terminal-rpm-rpm/actions/runs?per_page=5" \
